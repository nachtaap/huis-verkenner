...
// Toon de populariteitsindex sectie
...
// FUNDA POPULARITEITSINDEX FUNCTIES

// Populariteitsindex berekenen
function calculatePopularityIndex() {
...
if (views < 10) {
    alert('Minimaal 10 views nodig voor betrouwbare index');
    return;
}

// Berekeningen
const saveToViewRatio = (saves / views) * 100;
const dailyViews = views / daysOnline;

// Populariteitsindex formule: save ratio × log van dagelijkse views × factor
const popularityIndex = saveToViewRatio * Math.log10(dailyViews + 1) * 10;

// Status bepalen
let status, statusClass;
if (popularityIndex >= 50) {
    status = 'Uitstekend';
    statusClass = 'status-excellent';
} else if (popularityIndex >= 25) {
    status = 'Goed';
    statusClass = 'status-good';
} else if (popularityIndex >= 10) {
    status = 'Gemiddeld';
...
// Resultaten tonen
document.getElementById('popularityScore').innerHTML = 
    `<span class="${statusClass}">${popularityIndex.toFixed(1)}</span>`;

document.getElementById('saveRatio').textContent = `${saveToViewRatio.toFixed(1)}%`;
document.getElementById('dailyViews').textContent = dailyViews.toFixed(1);
document.getElementById('popularityStatus').innerHTML = 
    `<span class="${statusClass}">${status}</span>`;

// Resultaat sectie tonen met animatie
...
// Input validatie en real-time feedback
function initializePopularityValidation() {
...
this.title = ‘Minimaal 10 views voor betrouwbare index’;
} else {
...
// Functie om populariteitsindex sectie te tonen
function showPopularitySection() {
const section = document.getElementById(‘popularitySection’);
if (section) {
section.style.display = ‘block’;
// Initialiseer validatie als dat nog niet gebeurd is
if (!section.dataset.initialized) {
initializePopularityValidation();
section.dataset.initialized = ‘true’;
}
}
}

// Functie om populariteitsindex sectie te verbergen
function hidePopularitySection() {
const section = document.getElementById(‘popularitySection’);
if (section) {
section.style.display = ‘none’;
// Reset form
const viewsInput = document.getElementById(‘viewsInput’);
const savesInput = document.getElementById(‘savesInput’);
...
// Automatisch initialiseren als de DOM geladen is
document.addEventListener(‘DOMContentLoaded’, function() {
// Wacht even voor het geval andere scripts nog bezig zijn
setTimeout(initializePopularityValidation, 100);
});